...
namespace PlayWay.Water
{
	...
	public class WaterWavesParticleSystem : MonoBehaviour, IOverlaysRenderer
	{
		...
		private void UpdateLinearParticleData(int index)
		{
			...
			var particleData = new Vector4(particle.direction.x / particle.frequency, particle.direction.y / particle.frequency, particle.width);
		...
		sealed public class LinearParticle
		{
			...
			{
				float tanh = Mathf.Max(0.05f, (float)System.Math.Tanh(baseFrequency * depth));
				frequency = baseFrequency / tanh;
				amplitude = baseAmplitude / tanh;
				speed = 30.0f * Mathf.Sqrt(Physics.gravity.magnitude * frequency * (float)System.Math.Tanh(frequency * depth));
			}
		}
